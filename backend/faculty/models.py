...
class FacultyIncharge(models.Model):
    faculty = models.OneToOneField(Faculty, on_delete=models.CASCADE, related_name='incharge_role')
    class_group = models.OneToOneField(Class, on_delete=models.CASCADE, related_name='incharge')
...
class FacultyTimetable(models.Model):
    ...
    def __str__(self):
        return f"{self.faculty.name} - {self.class_group.name} - {self.day} {self.period}"
